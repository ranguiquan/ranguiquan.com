...
async function fetchAndRetryIfNecessary(callAPIFn) {
  ...
  try {
    ...
    if ([APIErrorCode.RateLimited, ClientErrorCode.RequestTimeout].includes(error.code)) {
      await sleep(1000);
 ...
 */
export async function retrieve_block_children({ block_id, page_size, start_cursor }) {
  return fetchAndRetryIfNecessary(() => {
    return requestRateLimiter.acquireToken(() => {
...
export async function query_comment_page_list({ database_id, page_id }) {
  return fetchAndRetryIfNecessary(() => {
    return requestRateLimiter.acquireToken(() => {
      return notion.databases.query({
        database_id: database_id,
...
export async function create_page({ parent, properties, children, icon, cover }) {
  return fetchAndRetryIfNecessary(() => {
    return requestRateLimiter.acquireToken(() => {
